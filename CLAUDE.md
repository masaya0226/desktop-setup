...
| 用語 | 意味 |
|---|---|
| **メインモニタ** | 物理配置で**右側**のモニタ。PBPなし。常に一つのPCのみを表示。macOS の主ディスプレイに固定。 |
| **サブモニタ** | 物理配置で**左側**のモニタ。PBP機能を使い2つのPCを並置可能。 |
| **主ディスプレイ** | macOS の「メインディスプレイ」。メニューバー・Dock が表示される。**メインモニタ = 主ディスプレイ** になるよう手動でシステム設定から固定する。 |
| **メインPC** | 現在 Corne キーボードで操作している側の Mac（物理 KVM で切替）。メインモニタに表示されている PC。 |
| **他PC** | メインPC ではない側の Mac。PBPオン時はサブモニタの左半分に表示。PBPオフ時は表示されない。 |
